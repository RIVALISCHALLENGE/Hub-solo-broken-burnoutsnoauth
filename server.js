...
app.get('/api/stripe/products', async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT p.id as product_id, p.name as product_name, p.description as product_description, pr.id as price_id, pr.unit_amount, pr.currency, pr.recurring FROM stripe.products p LEFT JOIN stripe.prices pr ON pr.product = p.id AND pr.active = true WHERE p.active = true');
    await pool.end();
    const products = [];
    result.rows.forEach(row => {
      let p = products.find(x => x.id === row.product_id);
      if (!p) { p = { id: row.product_id, name: row.product_name, description: row.product_description, prices: [] }; products.push(p); }
      if (row.price_id) p.prices.push({ id: row.price_id, unitAmount: row.unit_amount, currency: row.currency, recurring: row.recurring });
    });
    res.json({ products });
  } catch (error) { res.status(500).json({ error: 'Failed' }); }
});
...
app.get('/api/stripe/subscription', verifyFirebaseToken, async (req, res) => {
  try {
    ...
    if (!userDoc.exists || !userDoc.data().stripeCustomerId) return res.json({ subscription: null });
    const { Pool } = require('pg');
    const pool = new Pool({ connectionString: process.env.DATABASE_URL, max: 2 });
    const result = await pool.query('SELECT id, status, current_period_end FROM stripe.subscriptions WHERE customer = $1 AND status IN (\'active\', \'trialing\', \'past_due\') ORDER BY current_period_end DESC LIMIT 1', [userDoc.data().stripeCustomerId]);
    await pool.end();
    const sub = result.rows[0] || null;
    await db.collection('users').doc(req.user.uid).set({ subscriptionStatus: sub ? 'active' : 'inactive', subscriptionUpdatedAt: new Date() }, { merge: true });
